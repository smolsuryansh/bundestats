                  ...
                  <tbody>
                    {currentFilteredMatches.map((match, index) => (
                      <tr key={index}>
                        ...
                        <th className='border px-2 py-2'>
                          <a href={`https://google.com/search?q=${match.homeTeam.name} vs ${match.awayTeam.name} ${new Date(match.utcDate).toISOString().split('T')[0]}`} className='text-blue-500 underline'>View details</a>
                        </th>
                      </tr>
                    ))}
                  </tbody>
